...
5. **Run the local/root orchestrator with the full-tool harness profile.** The example agent frontmatter grants
   `read`, `search`, `edit`, `terminal`, `github`, `actions`, and `workiq` so local runs can perform Teams/M365
   intake, `gh`/`git`/test commands, GitHub polling, and harness artifact updates themselves. If your host uses
   different names for equivalent M365/terminal tools, map them during instantiation; do not leave the
   orchestrator with read/search-only access.
6. **Run workspace hygiene before every lifecycle attempt:** fetch the remote default branch, confirm the
   checkout is not stale/diverged, and quarantine old generated `.harness` artifacts before trusting an
   existing plan/dispatch/unit artifact.
7. **Turn on enforcement BEFORE the first unit Issue/PR** (the caveat above): vendor the workflows, add
...
8. **Drive the lifecycle:** local planning → rubber-duck → human approval → **`plan-to-issues`** (create the
   GitHub Issues) → dispatch each Issue to a dev-fleet agent or **Copilot cloud agent** → gated PRs.
9. Set the white-label variables (`{{DEMO_APP}}`, `{{DEPLOY_TARGET}}`, `{{FLEET_CONCURRENCY}}`, …) per
